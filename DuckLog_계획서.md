...
| OCR | **Gemini 2.0 Flash** | 영수증 사진 → 금액/품목/날짜 추출. 저비용($0.10/1M tokens), 한국어 우수 |
...
- **영수증 촬영**: 사진 → Gemini Vision → 금액/매장명/날짜 자동 추출 → 유저 확인/수정 후 저장
...
### Phase 3: 영수증 OCR (1주)
- [ ] 영수증 촬영 UI
- [ ] Gemini Vision API 연동 (Supabase Edge Function)
- [ ] 추출 결과 미리보기 + 수정 화면 ("이 정보가 맞나요?")
- [ ] 확인 후 goods 테이블에 저장
...
### Gemini Vision 프롬프트 (Edge Function에서 호출)
```
이 영수증 사진에서 다음 정보를 JSON으로 추출해줘:
{
  "store_name": "매장명",
  "purchased_at": "YYYY-MM-DD",
  "total_amount": 숫자(원),
  "items": [
    {"name": "품목명", "price": 숫자(원), "quantity": 숫자}
  ]
}
인식이 불확실한 필드는 null로 표시해.
```
...
### 원칙
- **광고 없음** (유저 경험 최우선)
- 개발자에게 실비가 드는 것만 유료화 — 비용 안 드는 기능은 전부 무료
- 가능한 것을 인위적으로 불가능하게 만들지 않음

### 실제 비용이 드는 항목
| 항목 | 비용 |
|------|------|
| Gemini OCR 호출 | 건당 ~10~20원 |
...
**1. OCR 스캔 크레딧 (소모품 인앱 구매)**
- 무료: 매월 5회 OCR 제공 (매월 리셋)
- 유료 크레딧 팩:
  - 10회 → 1,000원
  - 30회 → 2,500원
  - 100회 → 6,900원
- 이유: Gemini API 호출에 실비가 발생. 유저도 납득 가능

**2. 서포터 배지 (일회성 인앱 구매, 2,900원)**
- 프로필에 서포터 배지 표시
- "이 앱을 응원합니다" 느낌의 팁 jar
- 디자이너 불필요 (이모지/심플 아이콘 수준)

**3. 데이터 내보내기 (일회성 인앱 구매, 1,500원)**
- 내 덕질 기록 전체를 CSV/엑셀로 내보내기
- 파워 유저 대상, 코드만으로 구현 가능
...
### 수익화 타임라인
```
출시 초기 (1~2개월):  완전 무료. 유저 확보에 집중.
출시 후 2~3개월:      OCR 크레딧 + 서포터 배지 도입.
출시 후 4~6개월:      데이터 내보내기 추가.
...
| 항목 | 대응 |
|------|------|
| API 키 노출 방지 | Gemini, IGDB 키는 Supabase **Edge Function에서만 호출** (클라이언트에 노출 안 됨) |
...
- 제3자 제공: Supabase(DB/Storage), Google(인증), Kakao(인증), Google Gemini(OCR)
...
- 청약철회 (한국 전자상거래법):
  - OCR 크레딧: 미사용분 환불 가능, 사용 후 환불 불가 → 구매 전 고지 필수
  - 서포터 배지/내보내기: 디지털 콘텐츠로 제공 즉시 청약철회 제한 → 동의 후 구매
...
| Edge Functions | 50만 호출/월 | OCR + cron 감안 시 유저 3,000명+ |
| 비활성 중지 | 7일 미사용 시 | 개발 중에만 해당. 출시 후 무관 |

### Pro 전환 기준
- Storage 80% 도달 시 → Pro($25/월) 전환
- 이 시점이면 OCR 크레딧 수익으로 서버비 커버 가능한 규모
...
3. **OCR**: 실제 편의점/마트 영수증 5장 이상으로 추출 정확도 테스트
